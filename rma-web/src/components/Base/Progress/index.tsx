...
type ProgressProps = React.PropsWithChildren & React.ComponentPropsWithoutRef<'div'>;

function Progress(props: ProgressProps) {
  return (
    <div
      {...props}
      className={twMerge(['w-full h-2 bg-slate-200 rounded dark:bg-black/20', props.className])}
    >
      {props.children}
    </div>
  );
